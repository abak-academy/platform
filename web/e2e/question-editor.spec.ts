...
test.describe("question editor toolbar — caret/selection defects", () => {
  ...
  test("FB-24 — line breaks and paragraphs survive a save (P0, data loss)", async ({ page, context, request }) => {
    ...
    if (!identifier || !password) {
      throw new Error(
        "FB-24 exercises the real backend save/reload round trip and needs a real session: " +
          "set E2E_ADMIN_IDENTIFIER and E2E_ADMIN_PASSWORD to a local admin_exam or super_admin " +
          "account before running this spec. This is an environment/credentials gap, not the FB-24 bug."
      );
    }
    ...
    if (!loginRes.ok()) {
      throw new Error(
        `FB-24 setup: login failed (${loginRes.status()}) — check E2E_ADMIN_IDENTIFIER/E2E_ADMIN_PASSWORD. ` +
    ...
    await page.locator("#question-format").selectOption("essay");
    const topicOptionCount = await page.locator("#question-topic option").count();
    expect(
      topicOptionCount,
      "no exam topics exist in the local database — seed at least one to run FB-24"
    ).toBeGreaterThan(1);
