...
func TestListSchoolOptions_includesSearchMetadata(t *testing.T) {
	pool := newGradingTestPool(t)
	repo := New(pool)
	ctx := context.Background()

	suffix := uuid.New().String()[:8]
	schoolID := seedSchoolRow(t, repo, "Metadata Option "+suffix, "mo_"+uuid.New().String()[:10], "active")

	var cityID, cityName, provinceID, provinceName string
	require.NoError(t, pool.QueryRow(ctx, `
		...
		ORDER BY c.id
		LIMIT 1
	`).Scan(&cityID, &cityName, &provinceID, &provinceName))

	_, err := pool.Exec(ctx,
		`UPDATE school SET npsn = $1, alamat = $2, category = $3, city_id = $4 WHERE id = $5`,
		"20123456", "Jl. Metadata", "SMA", cityID, schoolID,
	)
	require.NoError(t, err)
	...
	var option SchoolOption
	found := false
	for _, o := range options {
		if o.ID == schoolID.String() {
			option = o
			found = true
			break
		}
	}
	require.True(t, found, "seeded school should be returned")

	require.Equal(t, "20123456", *option.NPSN)
	require.Equal(t, "Jl. Metadata", *option.Alamat)
	require.Equal(t, "SMA", *option.Category)
	require.Equal(t, cityID, *option.CityID)
	require.Equal(t, cityName, *option.CityName)
	require.Equal(t, provinceID, *option.ProvinceID)
	require.Equal(t, provinceName, *option.ProvinceName)
	require.Equal(t, "active", option.Status)
}
