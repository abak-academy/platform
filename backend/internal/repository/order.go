...
func orderFilterPredicateFrom(filter OrderFilter, startArg int) (string, []interface{}, int) {
	...
	// "all" is the frontend's sentinel for its own "All" filter tab, not a real
	// status value — treated as no filter here so it can't be forwarded as a
	// literal that matches zero rows.
